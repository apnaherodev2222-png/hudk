...
if not TOKEN:
    raise RuntimeError('BOT_TOKEN environment variable is required. Do NOT hardcode the Telegram bot token.')
OWNER_ID = 5628671567 # Replace with your Owner ID

# --- Docker Sandbox ---
SANDBOX_IMAGE_PY = os.environ.get("SANDBOX_PY_IMAGE", "python:3.11-slim")
SANDBOX_IMAGE_JS = os.environ.get("SANDBOX_JS_IMAGE", "node:20-bookworm-slim")
SANDBOX_NETWORK = os.environ.get("SANDBOX_NETWORK", "bridge")  # use 'none' for maximum isolation
SANDBOX_CPUS = os.environ.get("SANDBOX_CPUS", "1.0")
SANDBOX_MEMORY = os.environ.get("SANDBOX_MEMORY", "512m")
SANDBOX_PIDS = os.environ.get("SANDBOX_PIDS", "128")
SANDBOX_TMP_SIZE = os.environ.get("SANDBOX_TMP_SIZE", "128m")
MAX_PROJECT_BYTES = 50 * 1024 * 1024
MAX_ZIP_UNCOMPRESSED = 100 * 1024 * 1024
MAX_ZIP_FILES = 500

def _safe_name(value):
    return re.sub(r"[^A-Za-z0-9_.-]", "_", value)[:80] or "script"
...
def sandbox_container_name(script_owner_id, file_name):
    return "pdfhost_%s_%s" % (script_owner_id, _safe_name(os.path.splitext(file_name)[0]))

def sandbox_base_args(container_name, user_folder, image):
    return [
        "docker", "run", "--rm", "--init", "--name", container_name,
        "--cpus", SANDBOX_CPUS, "--memory", SANDBOX_MEMORY, "--memory-swap", SANDBOX_MEMORY,
        "--pids-limit", SANDBOX_PIDS, "--ulimit", "nofile=256:256",
        "--cap-drop=ALL", "--security-opt", "no-new-privileges:true",
        "--read-only", "--tmpfs", f"/tmp:rw,nosuid,nodev,size={SANDBOX_TMP_SIZE}",
        "--network", SANDBOX_NETWORK,
        "-v", f"{os.path.abspath(user_folder)}:/workspace:rw",
        "-w", "/workspace", image
    ]

def sandbox_python_command(script_owner_id, file_name, user_folder):
    name = sandbox_container_name(script_owner_id, file_name)
    args = sandbox_base_args(name, user_folder, SANDBOX_IMAGE_PY)
    script = "/workspace/" + os.path.basename(file_name)
    shell = "set -e; mkdir -p /tmp/pkgs; if [ -f /workspace/requirements.txt ]; then python -m pip install --disable-pip-version-check --no-cache-dir --target /tmp/pkgs -r /workspace/requirements.txt; fi; PYTHONPATH=/tmp/pkgs python " + repr(script)
    return name, args + ["sh", "-c", shell]

def sandbox_js_command(script_owner_id, file_name, user_folder):
    name = sandbox_container_name(script_owner_id, file_name)
    args = sandbox_base_args(name, user_folder, SANDBOX_IMAGE_JS)
    script = "/workspace/" + os.path.basename(file_name)
    shell = "set -e; if [ -f /workspace/package.json ]; then mkdir -p /tmp/npmdeps; npm install --prefix /tmp/npmdeps --no-audit --no-fund; fi; export NODE_PATH=/tmp/npmdeps/node_modules; node " + repr(script)
    return name, args + ["sh", "-c", shell]
...
def kill_process_tree(process_info):
    """Stop a sandbox container/process and close its log file."""
    ...
    try:
        if container_name:
            ...
            logger.info("Stopped sandbox %s (%s)", container_name, script_key)
    except Exception as e:
        logger.error("Sandbox stop failed for %s: %s", script_key, e, exc_info=True)
...
def _start_sandbox(kind, script_path, script_owner_id, user_folder, file_name, message_obj_for_reply):
    script_key = f"{script_owner_id}_{file_name}"
    if not docker_available():
        bot.reply_to(message_obj_for_reply, "❌ Docker sandbox is unavailable on this server. Install/start Docker first.")
        return
    if is_bot_running(script_owner_id, file_name):
        bot.reply_to(message_obj_for_reply, f"⚠️ '{file_name}' is already running.")
        return
    if not os.path.isfile(script_path):
        bot.reply_to(message_obj_for_reply, f"❌ Script '{file_name}' not found.")
        return
    log_file_path = os.path.join(user_folder, f"{os.path.splitext(os.path.basename(file_name))[0]}.log")
    try:
    ...
    try:
        if kind == 'py':
            container_name, command = sandbox_python_command(script_owner_id, file_name, user_folder)
        else:
            container_name, command = sandbox_js_command(script_owner_id, file_name, user_folder)
        logger.info("Starting %s sandbox: %s", kind, container_name)
        process = subprocess.Popen(command, cwd=BASE_DIR, stdout=log_file, stderr=log_file, stdin=subprocess.DEVNULL, text=True)
        bot_scripts[script_key] = {
            'process': process, 'log_file': log_file, 'file_name': file_name,
            'chat_id': message_obj_for_reply.chat.id, 'script_owner_id': script_owner_id,
            'start_time': datetime.now(), 'user_folder': user_folder, 'type': kind,
            'script_key': script_key, 'container_name': container_name, 'sandbox': True
        }
        bot.reply_to(message_obj_for_reply, f"✅ {kind.upper()} script '{file_name}' started in Docker sandbox.\n🧱 CPU: {SANDBOX_CPUS} | RAM: {SANDBOX_MEMORY} | PIDs: {SANDBOX_PIDS}")
    except Exception as e:
        try: log_file.close()
        except Exception: pass
        logger.error("Sandbox start failed for %s: %s", script_key, e, exc_info=True)
        bot.reply_to(message_obj_for_reply, f"❌ Failed to start sandbox: {e}")

def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    _start_sandbox('py', script_path, script_owner_id, user_folder, file_name, message_obj_for_reply)

def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    _start_sandbox('js', script_path, script_owner_id, user_folder, file_name, message_obj_for_reply)
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        file_name_zip = os.path.basename(file_name_zip or "project.zip")
        zip_path = os.path.join(temp_dir, file_name_zip)
        with open(zip_path, 'wb') as new_file: new_file.write(downloaded_file_content)
        if len(downloaded_file_content) > MAX_PROJECT_BYTES:
            raise ValueError("ZIP/file exceeds the 50 MB upload limit")
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            members = zip_ref.infolist()
            if len(members) > MAX_ZIP_FILES:
                raise zipfile.BadZipFile("ZIP contains too many files")
            ...
            for member in members:
                ...
                if total_uncompressed > MAX_ZIP_UNCOMPRESSED:
                    raise zipfile.BadZipFile("ZIP uncompressed size exceeds 100 MB")
        ...
        if req_file:
            logger.info("requirements.txt detected; dependencies will be installed inside the Docker sandbox, never on the host.")
            bot.reply_to(message, "🧱 requirements.txt detected. Dependencies will be installed inside the isolated Docker sandbox.")

        if pkg_json:
            logger.info("package.json detected; npm dependencies will be installed inside the Docker sandbox, never on the host.")
            bot.reply_to(message, "🧱 package.json detected. Node dependencies will be installed inside the isolated Docker sandbox.")
...
def process_add_admin_id(message):
    ...
    if (message.text or '').lower() == '/cancel': bot.reply_to(message, "Admin promotion cancelled."); return
    try:
...
def process_remove_admin_id(message):
    owner_id_check = message.from_user.id
    if owner_id_check != OWNER_ID: bot.reply_to(message, "⚠️ Owner only."); return
    if (message.text or '').lower() == '/cancel': bot.reply_to(message, "Admin removal cancelled."); return
    try:
...
def process_add_subscription_details(message):
    ...
    if (message.text or '').lower() == '/cancel': bot.reply_to(message, "Sub add cancelled."); return
    try:
...
def process_remove_subscription_id(message):
    admin_id_check = message.from_user.id
    if admin_id_check not in admin_ids: bot.reply_to(message, "⚠️ Not authorized."); return
    if (message.text or '').lower() == '/cancel': bot.reply_to(message, "Sub removal cancelled."); return
    try:
...
def process_check_subscription_id(message):
    admin_id_check = message.from_user.id
    if admin_id_check not in admin_ids: bot.reply_to(message, "⚠️ Not authorized."); return
    if (message.text or '').lower() == '/cancel': bot.reply_to(message, "Sub check cancelled."); return
    try:
