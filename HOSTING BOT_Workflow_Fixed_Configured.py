...
# --- Configuration ---
TOKEN = '7384442199:AAGq-XNYgE6UDrgtPwnmc_4wwJ-ETSBTH2E'
OWNER_ID = 7265678519
ADMIN_ID = 7265678519
...
def _find_missing_python_module(script_path, user_folder):
    """Statically find the first missing third-party Python import without executing the script."""
    try:
        with open(script_path, 'r', encoding='utf-8', errors='ignore') as fh:
            tree = ast.parse(fh.read(), filename=script_path)
        local_names = {os.path.splitext(os.path.basename(x))[0] for x in os.listdir(user_folder)
                       if x.endswith('.py')}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                continue
            for name in names:
                if name in seen or name in stdlib or name in local_names:
                    continue
                seen.add(name)
                if importlib.util.find_spec(name) is None:
                    return name
    except (SyntaxError, OSError, ValueError) as exc:
        logger.warning(f'Could not statically inspect Python imports in {script_path}: {exc}')
    return None

def _find_missing_node_module(script_path, user_folder):
    """Statically inspect common Node imports and resolve them without executing the uploaded script."""
    try:
        with open(script_path, 'r', encoding='utf-8', errors='ignore') as fh:
            source = fh.read()
        names = []
        names += re.findall(r"(?:require\s*\(\s*|from\s+|import\s+)['\"]([^'\"]+)['\"]", source)
        for name in names:
            if name.startswith('.') or name.startswith('/') or name in {'node:fs','node:path','node:os','node:http','node:https','node:url','node:util','node:events','node:crypto','node:stream'}:
                continue
            result = subprocess.run(['node', '-e', 'require.resolve(process.argv[1])', name], cwd=user_folder,
                                    stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=5)
            if result.returncode != 0:
                return name.split('/')[0] if not name.startswith('@') else '/'.join(name.split('/')[:2])
    except (OSError, subprocess.SubprocessError) as exc:
        logger.warning(f'Could not statically inspect Node imports in {script_path}: {exc}')
    return None
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            logger.info(f"Running Python syntax pre-check: {' '.join(check_command)}")
            check_proc = None
            try:
                check_proc = subprocess.Popen(check_command, cwd=user_folder, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore')
                stdout, stderr = check_proc.communicate(timeout=10)
                if check_proc.returncode != 0:
                    error_summary = (stderr or stdout)[:500]
                    bot.reply_to(message_obj_for_reply, f"❌ Syntax error in '{file_name}':\n```\n{error_summary}\n```", parse_mode='Markdown')
                    return
                module_name = _find_missing_python_module(script_path, user_folder)
                if module_name:
                    logger.info(f"Detected missing Python module without executing script: {module_name}")
                    if attempt_install_pip(module_name, message_obj_for_reply):
                        bot.reply_to(message_obj_for_reply, f"🔄 Install successful. Retrying '{file_name}'...")
                        time.sleep(2)
                        threading.Thread(target=run_script, args=(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt + 1), daemon=True).start()
                        return
                    bot.reply_to(message_obj_for_reply, f"❌ Install failed. Cannot run '{file_name}'.")
                    return
            except subprocess.TimeoutExpired:
                logger.warning("Python syntax pre-check timed out (>10s); stopping check process.")
                ...
                logger.error(f"Error in Python syntax pre-check for {script_key}: {e}", exc_info=True)
                bot.reply_to(message_obj_for_reply, f"❌ Unexpected error checking '{file_name}': {e}")
                return
            finally:
                if check_proc and check_proc.poll() is None:
                    check_proc.kill(); check_proc.communicate()
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            logger.info(f"Running JS syntax pre-check: {' '.join(check_command)}")
            check_proc = None
            try:
                check_proc = subprocess.Popen(check_command, cwd=user_folder, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore')
                stdout, stderr = check_proc.communicate(timeout=10)
                if check_proc.returncode != 0:
                    ...
                    bot.reply_to(message_obj_for_reply, f"❌ Syntax error in '{file_name}':\n```\n{error_summary}\n```", parse_mode='Markdown')
                    return
                module_name = _find_missing_node_module(script_path, user_folder)
                if module_name:
                    logger.info(f"Detected missing Node module without executing script: {module_name}")
                    if attempt_install_npm(module_name, user_folder, message_obj_for_reply):
                        bot.reply_to(message_obj_for_reply, f"🔄 NPM install successful. Retrying '{file_name}'...")
                        time.sleep(2)
                        threading.Thread(target=run_js_script, args=(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt + 1), daemon=True).start()
                        return
                    bot.reply_to(message_obj_for_reply, f"❌ NPM install failed. Cannot run '{file_name}'.")
                    return
            except subprocess.TimeoutExpired:
                logger.warning("JS syntax pre-check timed out (>10s); stopping check process.")
                if check_proc and check_proc.poll() is None:
                    check_proc.kill(); check_proc.communicate()
                return
            except FileNotFoundError:
                bot.reply_to(message_obj_for_reply, "❌ Error: 'node' not found. Ensure Node.js is installed for JS files.")
                return
            except Exception as e:
                logger.error(f"Error in JS syntax pre-check for {script_key}: {e}", exc_info=True)
                bot.reply_to(message_obj_for_reply, f"❌ Unexpected error checking '{file_name}': {e}")
                return
            finally:
                if check_proc and check_proc.poll() is None:
