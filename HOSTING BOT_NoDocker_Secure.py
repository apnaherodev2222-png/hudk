...
YOUR_USERNAME = os.environ.get('YOUR_USERNAME', '@Dark_H4cker_07')
UPDATE_CHANNEL = os.environ.get('UPDATE_CHANNEL', '@dark_x_tech')
if not TOKEN:
...
# No-Docker hardening. This reduces risk but is NOT a true security sandbox.
MAX_RUNTIME_SECONDS = int(os.environ.get('MAX_RUNTIME_SECONDS', '86400'))
MAX_CPU_SECONDS = int(os.environ.get('MAX_CPU_SECONDS', '900'))
MAX_MEMORY_MB = int(os.environ.get('MAX_MEMORY_MB', '768'))
MAX_OUTPUT_MB = int(os.environ.get('MAX_OUTPUT_MB', '100'))
MAX_PROCESSES = int(os.environ.get('MAX_PROCESSES', '64'))
START_COOLDOWN_SECONDS = int(os.environ.get('START_COOLDOWN_SECONDS', '10'))
MAX_CONCURRENT_PER_USER = int(os.environ.get('MAX_CONCURRENT_PER_USER', '2'))
MAX_ZIP_FILES = int(os.environ.get('MAX_ZIP_FILES', '200'))
MAX_ZIP_UNCOMPRESSED_MB = int(os.environ.get('MAX_ZIP_UNCOMPRESSED_MB', '100'))
MAX_ZIP_RATIO = int(os.environ.get('MAX_ZIP_RATIO', '100'))
...
# --- Helper Functions ---
def safe_filename(name, allowed_exts=None):
...
def safe_user_path(user_folder, name):
    name = safe_filename(name)
    base = os.path.realpath(user_folder)
    candidate = os.path.realpath(os.path.join(base, name))
    if os.path.commonpath([base, candidate]) != base:
        raise ValueError('Unsafe path')
    return candidate

def can_start_script(user_id):
    now = time.monotonic()
    with start_lock:
        last = last_start_times.get(user_id, 0.0)
        if now - last < START_COOLDOWN_SECONDS:
            return False, max(1, int(START_COOLDOWN_SECONDS - (now - last)))
        running = sum(1 for info in bot_scripts.values() if info.get('script_owner_id') == user_id and info.get('process') and info['process'].poll() is None)
        if user_id not in admin_ids and running >= MAX_CONCURRENT_PER_USER:
            return False, 'limit'
        last_start_times[user_id] = now
    return True, 0

def runner_environment():
    """Remove bot secrets from uploaded child processes."""
    env = os.environ.copy()
    for key in ('BOT_TOKEN', 'OWNER_ID', 'ADMIN_ID'):
        env.pop(key, None)
...
def apply_process_limits():
    ...
    limits = [
        ...
        (resource.RLIMIT_FSIZE, MAX_OUTPUT_MB * 1024 * 1024),
    ...
    for limit, value in limits:
        try:
            ...
            logger.warning('Could not apply resource limit %s: %s', limit, exc)

def watch_process(script_key, process, log_file):
    deadline = time.monotonic() + MAX_RUNTIME_SECONDS
    while process.poll() is None:
        if time.monotonic() >= deadline:
            logger.warning('Runtime limit reached for %s (PID %s)', script_key, process.pid)
...
def attempt_install_pip(module_name, message):
    if os.environ.get('ALLOW_AUTO_INSTALL', '0') != '1':
        bot.reply_to(message, '⚠️ Automatic Python package installation is disabled for security.')
        return False
...
def attempt_install_npm(module_name, user_folder, message):
    if os.environ.get('ALLOW_AUTO_INSTALL', '0') != '1':
        bot.reply_to(message, '⚠️ Automatic Node package installation is disabled for security.')
        return False
    try:
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        allowed, wait = can_start_script(script_owner_id)
        if not allowed:
            bot.reply_to(message_obj_for_reply, f"⏳ Please wait {wait}s before starting another script." if wait != 'limit' else f"⚠️ Maximum {MAX_CONCURRENT_PER_USER} concurrent scripts allowed.")
            return
        logger.info(f"Starting long-running Python process for {script_key}")
        log_file_path = os.path.join(user_folder, f"{os.path.splitext(file_name)[0]}.log")
        log_file = None; process = None
        try: log_file = open(log_file_path, 'a', encoding='utf-8', errors='ignore')
        except Exception as e:
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                preexec_fn=apply_process_limits if os.name == 'posix' else None,
            ...
            threading.Thread(target=watch_process, args=(script_key, process, log_file), daemon=True).start()
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            if len(infos) > MAX_ZIP_FILES:
                raise zipfile.BadZipFile(f'ZIP contains too many files (max {MAX_ZIP_FILES}).')
            ...
            for member in infos:
                ...
                if total_uncompressed > MAX_ZIP_UNCOMPRESSED_MB * 1024 * 1024:
                    raise zipfile.BadZipFile('ZIP expands beyond the allowed size.')
                if member.compress_size and member.file_size / member.compress_size > MAX_ZIP_RATIO:
                    raise zipfile.BadZipFile(f'ZIP compression ratio too high: {member.filename}')
                member_path = os.path.realpath(os.path.join(base, member.filename))
        ...
        if req_file and os.environ.get('ALLOW_AUTO_INSTALL', '0') == '1':
        ...
        if req_file and os.environ.get('ALLOW_AUTO_INSTALL', '0') != '1':
            bot.reply_to(message, '⚠️ requirements.txt detected, but automatic installation is disabled for security. Dependencies were not installed.')
        if pkg_json and os.environ.get('ALLOW_AUTO_INSTALL', '0') == '1':
        ...
        if pkg_json and os.environ.get('ALLOW_AUTO_INSTALL', '0') != '1':
            bot.reply_to(message, '⚠️ package.json detected, but automatic npm installation is disabled for security.')
        ...
        for item_name in os.listdir(temp_dir):
            ...
            dest_path = safe_user_path(user_folder, item_name)
...
def handle_file_upload_doc(message): # Renamed
    ...
    try:
        file_name = safe_filename(file_name, {'.py', '.js', '.zip'})
    except ValueError:
        bot.reply_to(message, '⚠️ Invalid or unsafe filename.')
        return
...
def restart_bot_callback(call):
    try:
        ...
        file_path = safe_user_path(user_folder, file_name); script_key = f"{script_owner_id}_{file_name}"
...
def delete_bot_callback(call):
    try:
        ...
        user_folder = get_user_folder(script_owner_id)
        file_path = safe_user_path(user_folder, file_name)
        log_path = safe_user_path(user_folder, f"{os.path.splitext(safe_filename(file_name))[0]}.log")
        deleted_disk = []
        if os.path.exists(file_path):
